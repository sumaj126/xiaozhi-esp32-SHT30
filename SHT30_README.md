...
ESP32会定期将温湿度数据上传到云服务器，实现远程监控。

**上传参数：**
- 上传URL：`http://175.178.158.54:7791/update`
- 设备ID：`pcroom-esp32`
- 上传间隔：待机状态10秒，其他状态不上传

**上传数据格式：**
```json
{
  "device_id": "pcroom-esp32",
  "temperature": 23.5,
  "humidity": 45.2,
...
`device_status` 字段表示设备当前状态：

| 状态码 | 状态名 | 说明 |
|--------|--------|------|
| 0 | 待机 | 设备空闲，正常上传温湿度数据 |
| 1 | 唤醒中 | 检测到唤醒词，正在激活语音助手 |
| 2 | 录音中 | 正在录制用户语音输入 |
| 3 | 播放中 | 正在播放AI回复语音 |
| 4 | 配置中 | 正在配置WiFi或进行其他配置 |

### 网页监控页面

访问 `http://175.178.158.54:7791` 可查看实时温湿度监控页面。

**页面功能：**
- 实时显示温度和湿度
- 显示设备当前状态（在线/离线/唤醒中/录音中/播放中/配置中）
- 显示最后更新时间
- 自动刷新数据（每2秒）
- 响应式设计，支持手机访问
...
**优先级设计：**
温湿度上传优先级最低，确保不影响语音助手功能：
- 待机状态：每10秒上传一次
- 唤醒/录音/播放/配置状态：暂停定时上传，仅在状态变化时上传一次

### 启动立即上传

ESP32启动后会立即上传一次温湿度数据，减少网页显示"等待首次数据"的时间。

### 服务器架构

服务器端使用Node.js + HTTP实现：
- `/update` 接口：接收ESP32上传的数据（POST）
- `/api/data` 接口：返回最新数据供网页查询（GET）
- `/` 接口：返回监控网页（GET）

详见 `SERVER_ARCHITECTURE.md` 和 `SENSOR_UPLOAD_README.md`

## 版本历史

### v1.1.0 (2026-02-12)
- 添加云端数据上传功能
- 实现设备状态上传（唤醒中/录音中/播放中/配置中）
- 优化上传优先级，确保不影响语音助手
- 修复网页显示问题（移动端居中、等待数据问题）
- 添加服务器端设备状态显示
- 实现启动立即上传功能
